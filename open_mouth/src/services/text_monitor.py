...
# Agent "silence" patterns — the LLM writes these when it means to produce no output.
# These must NEVER be spoken aloud. Catch them here as a last line of defense.
_SILENCE_PATTERNS = re.compile(
    r'^\s*'
    r'('
    r'\[MOLT SPEAK: SILENT RESPONSE\]'
    r'|\(no output\)'
    r'|\(silence\)'
    r'|\(no response\)'
    r'|\(empty\)'
...
def _is_agent_silence(text: str) -> bool:
    """Check if text is an agent's attempt at 'silence' rather than real speech."""
    return bool(_SILENCE_PATTERNS.match(text))
...
class TextFileHandler(FileSystemEventHandler):
    ...
    def read_new_content(self) -> None:
        ...
        try:
            # Detect file truncation (barge-in clear signal from Ears)

            if current_size < self.last_position:
                self.last_position = 0
                return  # Don't read yet — wait for fresh content
            ...
            with open(self.file_path, "r", encoding="utf-8") as f:
                ...
                if new_content.strip():
                    ...
                    for line in lines:
                        line = line.strip()
                        if line:
                            if _is_agent_silence(line):
                                logger.debug(f"Suppressed agent silence: {line}")
                                break  # Discard rest of batch — agent intended silence
