...
**Version 1.1 Released** — The free truly conversational interface for Open Claw. Run `moltspeak update` for the latest.
...
| 1.1.0 | 2026-02-25 | **The conversational update** — Agent Boldness (adjustable conversation eagerness), Contextual Awareness via Social Cue Heuristics, Smart Silence (`[MOLT SPEAK: SILENT RESPONSE]`), Smart Bow-Out, Continue the Joke (barge-in resume — yes, it's a Boondock Saints reference. IYKYK), greatly reduced round-trip latency (Faster Whisper ~1s STT, sentence-level TTS), Debug Mode, new menu with lobster-robot button and continuous-value sliders for mic/barge-in sensitivity. All features tested on Haiku 4-5. |
